...
const mvcConfig = require(path.resolve('./mvc.config.js'));
...
function configurePackageJSON() {
    ...
    switch(framework){
        case'react':   configureReact(packageJSONPath,packageJSON);    break;
...
function configureReact(serverPackageJSONPath,serverPackageJSON) {
    ...
    //configure the build path for a create-react-app
    configureBuildPath('./client/build')
...
function addServerDependencies(packageJSON){
    const { dependencies, main } = packageJSON;
...
function configureBuildPath(newBuildPath){
    // const { buildPath } = mvcConfig;
    // if(!buildPath) mvcConfig.buildPath = newBuildPath;

    // fs.writeFileSync(path.resolve('./mvc.config.js'),mvcConfig);
}
