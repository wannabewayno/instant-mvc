...
function lookAhead(route,routeMap){
    ...
    if (primaryType === 'object'){
        ...
        if(someAreObjects) {
            // if some are objects, check to see if all are objects
            const allAreObjects = Object.keys(routeMap[route])
                .every(nestedRoute => findType(routeMap[route][nestedRoute]) === 'object' );

            if(allAreObjects) return 'directory';
            else return 'route directory';
